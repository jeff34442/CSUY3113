...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Project 4: Pokemon Battle!", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
    ...
    state.player->textureID = LoadTexture("charizard.png");
    ...
    state.enemy[0].textureID = LoadTexture("caterpie.png");
    state.enemy[1].textureID = LoadTexture("squirtle_squad.png");
    state.enemy[2].textureID = LoadTexture("zubat.png");
    state.enemy[0].position = glm::vec3(-3.5f, -1.0f, 0.0f);
    
    state.enemy[1].position = glm::vec3(3.5f, 1.5f, 0.0f);
    state.enemy[1].acceleration = glm::vec3(0.0f, -9.81f, 0.0f);
    
    state.enemy[2].position = glm::vec3(-3.0f, 0.0f, 0.0f);
    
    state.enemy[0].aiType = CATERPIE;
    state.enemy[1].aiType = SQUIRTLE;
    state.enemy[2].aiType = ZUBAT;
    
    state.enemy[1].height = 1.9f;
    state.enemy[1].width = 1.0f;
    
    state.enemy[2].height = 2.5f;
    state.enemy[2].width = 0.8f;
    
    state.enemy[3].height = 1.8f;
    state.enemy[3].width = 0.8f;
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_x:
                ...
                if ((fireCount < 3 || (fireCount == 3 && state.fire[fireIndex].isActive == false)) && state.player->isDead == false) {
                    ...
                    state.fire[fireIndex].textureID = LoadTexture("fireball.png");
...
void Update() {
    ...
    if (state.player->isDead == true) {
        state.player->textureID = LoadTexture("Charizard_sad.png");
        state.player->height = 1.9f;
    }

    if (state.enemy[0].isDead == true) {
        state.enemy[0].textureID = LoadTexture("sad_caterpie.png");
        state.enemy[0].height = 1.5f;
    }
    if (state.enemy[1].isDead == true) {
        state.enemy[1].textureID = LoadTexture("sad_squirtle.png");
    }
    if (state.enemy[2].isDead == true) {
        state.enemy[2].textureID = LoadTexture("sad_zubat.png");
        state.enemy[2].height = 1.9f;
    }
