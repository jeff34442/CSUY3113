...
void Map::Build()
{
    for (int y = 0; y < this->height; y++) {
        for (int x = 0; x < this->width; x++) {
            ...
            float fromX = -(tile_size / 2); // From center of tile
            float fromY = (tile_size / 2); // From center of tile

            vertices.insert(vertices.end(), {
            fromX + (tile_size * x), fromY + -tile_size * y,
            fromX + (tile_size * x), fromY + (-tile_size * y) - tile_size,
            fromX + (tile_size * x) + tile_size, fromY + (-tile_size * y) - tile_size,

            fromX + (tile_size * x), fromY + -tile_size * y,
            fromX + (tile_size * x) + tile_size, fromY + (-tile_size * y) - tile_size,
            fromX + (tile_size * x) + tile_size, fromY + -tile_size * y
                });
